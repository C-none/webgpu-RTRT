...
function alignTo(x, align) {
    return Math.floor((x + align - 1) / align) * align;
}
function createRenderPipeline(device, swapChainFormat) {
...
function createAndBuildRayTracingAccelerationContainer(device) {
...
async function createRayTracingPipeline(device, tlas) {
    const groups = [{
            type: 'general',
            generalIndex: 0,
        }, {
...
function createShaderBindingTable(device, pipeline) {
    const sbt = {
        rayGen: {},
        rayMiss: {},
        rayHit: {},
    ...
    // always start at 0
    sbt.rayGen.start = 0;
    sbt.rayGen.stride = alignTo(device.ShaderGroupHandleSize + /*maxRgenShaderRecordSize=*/ 0, device.ShaderGroupHandleAlignment);
    ...
    sbt.rayMiss.stride = alignTo(device.ShaderGroupHandleSize + /*maxRmissShaderRecordSize=*/ 0, device.ShaderGroupHandleAlignment);
    sbt.rayMiss.size = 0;
    sbt.rayHit.start = alignTo(sbt.rayMiss.start + sbt.rayMiss.size, device.ShaderGroupBaseAlignment);
    sbt.rayHit.stride = alignTo(device.ShaderGroupHandleSize + /*maxHitGroupShaderRecordByteSize=*/ 0, device.ShaderGroupHandleAlignment);
    ...
    });
    {
        ...
        const handle = groupHandles[groupHandleIndex /* num rgen + num rmiss */
            + hitShaderGroupIndex];
...
async function main(canvas) {
    ...
    function frame() {
        ...
        // ray tracing pass
        {
            ...
            passEncoder.setBindGroup(0, rayTracingBindGroup);
            passEncoder.traceRays(device, sbt, canvas.width, canvas.height);
            passEncoder.end();
        }
        // rasterization pass
        {
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        await main(document.querySelector('canvas'));
    }
    catch (e) {
        alert(e);
    }
